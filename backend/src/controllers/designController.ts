...
export class DesignController {
    ...
    async generate(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            const { images: generatedImages, analysis } = await aiService.generateRoomRedesign(
                design.originalImage,
                design.style,
                design.roomType,
                design.budget ?? undefined,
                1 // Only 1 variation requested by user
            );

            // 2. Generate a personalized AI Design Story (Consultation)
            const metadataJson = design.metadata as any || {};
            ...
            // Reset status to 'failed' so the user can retry
            try {
                const designId = typeof req.params.id === 'string' ? req.params.id : '';
                if (designId) {
                    await designRepository.updateStatus(designId, 'failed');
                }
            } catch (statusError) {
                logger.error('Failed to update design status to failed:', statusError);
            }
            next(error);
        }
    }
    ...
    async getGallery(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
        try {
            const limitInt = parseInt(req.query.limit as string) || 12;
    ...
    async getOptions(req: AuthenticatedRequest, res: Response): Promise<void> {
        res.json({
            success: true,
            data: {
    ...
    async getFloorPlan(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
        try {
            const { id } = req.params;
            const userId = req.userId!;
            const designId = typeof id === 'string' ? id : '';
            ...
            const floorPlan = await aiService.generateFloorPlan(
                design.originalImage,
                design.roomType
            );
